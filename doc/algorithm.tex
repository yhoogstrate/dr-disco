...
\end{tikzpicture}
\end{center}
\caption{Insert all splice junctions}
\label{fig:merge_sj_2}
\end{figure}
...
\begin{tikzpicture}[->,>=stealth',shorten >=1pt,auto,node distance=3cm,
  ...
  \node[main node] (3) [right of=4,color=gray] {\small{chr1:2400}};
  \node[main node] (2) [right of=3,color=gray] {\small{chr1:1250}};
  \node[main node] (1) [right of=2]{\small{chr1:1000}};
  ...
  \path[every node/.style={font=\sffamily\small}]
    ...
    (3) edge [color=gray,dotted] node {s-link} (4)
    (4) edge [color=gray,dotted] node {$d_2=100$} (3)
    (1) edge [bend left,color=black,dotted] node {effective distance $d=350$} (4)
    (4) edge [bend right,color=black,dotted] node {} (1)
;
\end{tikzpicture}
\caption{Extract SN based on SJ 02}
\label{fig:extract_sn_02}
\end{figure}
...
\end{tikzpicture}
\end{center}
\caption{Extract SN based on SJ 03}
\label{fig:extract_sn_03}
\end{figure}
